...
| Host Addons | `MKRBlenderSceneImport`, `MKRTouchDesignerImport`, `MKRTiXLImport`, `MKRNukeScriptImport`, `MKRPhotoshopDocumentImport`, `MKRAfterEffectsCompImport`, `MKRPremiereSequenceImport`, `MKRAffinityDocumentImport`, `MKRAffinityPhotoshopPluginPlan`, `MKRFusion360SceneImport`, `MKRFusion360ImageImport`, `MKRMayaSceneImport`, `MKRMayaImageImport`, `MKRNodeExtensionBuilderAdvanced`, `MKRNodeExtensionBuilderPlan`, `MKRNodeExtensionPyprojectPlan` | Packet-first host integrations with per-application addon scaffolds under `addons/` and matching ComfyUI nodes under `Addons/...`, including image/texture handoff lanes for the 3D hosts, an Affinity route based on Photoshop-plugin compatibility, plus an extension-builder lane with a cleaner advanced sidecar node for repository/license/tag metadata, manifest planning, and registry-ready `pyproject.toml` drafting |
...
| Mask + Utility | `x1MaskGen`, `AdvResize`, `MKRImageSplitGrid`, `MKRImageCombineGrid`, `MKRPixelPaletteReduce`, `MKRSpriteSheetExtract`, `xShader`, `x1DenoiseDetail` | Mask generation, resize work, tiled image workflows, pixel-art palette reduction, sprite extraction, shader utilities, and cleanup |
...
This repo now includes a starter builder config at `extension.builder.json` and a minimal example workflow at `example_workflows/mkrshift_extension_builder_plan.json` for `MKRNodeExtensionBuilderPlan`, the `MKRNodeExtensionBuilderAdvanced` sidecar node, and `MKRNodeExtensionPyprojectPlan`.
...
Then write the `builder_manifest_json` output from `MKRNodeExtensionBuilderPlan` to `extension.builder.json` and run your preferred builder CLI. For cleaner graphs, keep expert metadata on `MKRNodeExtensionBuilderAdvanced`, connect its `advanced_options_json` output into `advanced_bundle_json` on the plan node, and feed the resulting manifest into `MKRNodeExtensionPyprojectPlan` when you want a matching `pyproject.toml` draft.
