...
def _pack_ratio_defaults(pack: Dict[str, Any], platform: str) -> List[str]:
    ratios = [str(value).strip() for value in pack.get("defaults", {}).get("ratios", []) if str(value).strip()]
    ratios = _dedupe_keep_order(ratios)
    if ratios:
        return ratios
    return [str(_platform_value(platform).get("ratio_default", "4:5"))]


def _build_ratio_plan(aspect: str, output_mode: str, platform: str, pack: Dict[str, Any], count: int) -> Dict[str, Any]:
    item_count = int(max(1, count))
    explicit = str(aspect or "").strip()
    if explicit and explicit != "Auto":
        return {
            "aspect_used": explicit,
            "sequence": [explicit] * item_count,
            "strategy": "fixed_user",
            "ratios_used": [explicit],
        }

    pack_ratios = _pack_ratio_defaults(pack, platform)

    if str(output_mode) == "Story":
        return {
            "aspect_used": "9:16",
            "sequence": ["9:16"] * item_count,
            "strategy": "fixed_mode",
            "ratios_used": ["9:16"],
        }

    if str(output_mode) == "Mixed":
        sequence = [pack_ratios[idx % len(pack_ratios)] for idx in range(item_count)]
        return {
            "aspect_used": sequence[0],
            "sequence": sequence,
            "strategy": "per_asset_cycle" if len(pack_ratios) > 1 else "fixed_default",
            "ratios_used": pack_ratios,
        }

    return {
        "aspect_used": pack_ratios[0],
        "sequence": [pack_ratios[0]] * item_count,
        "strategy": "fixed_default",
        "ratios_used": [pack_ratios[0]],
    }
...
def _build_shot_plan(pack: Dict[str, Any], count: int, ratio_sequence: List[str]) -> List[Dict[str, Any]]:
        ...
        out.append(
            {
                ...
                "ratio": str(ratio_sequence[idx] if idx < len(ratio_sequence) else ratio_sequence[-1]),
            }
        )
    return out


def _asset_role(index: int, total: int) -> str:
    if index <= 0:
        return "hero"
...
class MKRshiftSocialPackBuilder:
    ...
    def build(
        ...
        **legacy_kwargs,
        ...
        aspect_used = str(ratio_plan["aspect_used"])
        has_logo = brand_logo is not None

        shot_plan = _build_shot_plan(pack_data, item_count, list(ratio_plan["sequence"]))
        ...
        for idx, shot_item in enumerate(shot_plan):
            ...
            ratio = str(shot_item.get("ratio", aspect_used))
            ...
            assets.append(
                {
                    ...
                    "shot": shot,
                    "hook": hook,
                    "role": _asset_role(idx, len(shot_plan)),
                    "ratio": ratio,
                    "slot": shot_item.get("slot", f"item_{idx + 1:02d}"),
                    "prompt": prompt,
                    "negative": negative,
                    "caption": caption,
                    "hashtags": hashtags,
        ...
        for idx, schedule_item in enumerate(schedule):
            if idx < len(shot_plan):
                shot_plan[idx]["publish_at_local"] = schedule_item.get("publish_at_local", "")
                shot_plan[idx]["platform"] = schedule_item.get("platform", platform)
            if idx < len(assets):
                assets[idx]["publish_at_local"] = schedule_item.get("publish_at_local", "")
                assets[idx]["platform"] = schedule_item.get("platform", platform)
        ...
        plan = {
            ...
            "generation": {
                ...
                "count": item_count,
                "aspect": aspect_used,
                "aspect_strategy": str(ratio_plan["strategy"]),
                "ratios_used": list(ratio_plan["ratios_used"]),
                "branding": branding,
                "seed_mode": seed_mode,
